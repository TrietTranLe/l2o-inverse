...
from data.eeg.utils_eeg import (load_fwd, load_mne_info,
                                   ...
                                   plot_src_from_imgs)
from data.eeg.utils_eeg import signal_to_windows, windows_to_signal
...
test_config = OmegaConf.load(
    str(Path('configs', 'dataset', args.test_data_config))
)
...
datamodule_conf = {
    ...
    "subset_name": "none", #args.subset_name, #
    "dl_kw":{
        "batch_size": 32
    },
...
for k in tqdm(range(n_val_samples)):
    ...
    ###
    for m in methods:
        inference_time_dict[m][k] = -time.process_time()
