...
class Esi_ModGrad_solver(nn.Module): 
    ...
    def forward(
        ...
        with torch.set_grad_enabled(True):
            ...
            for step in range(steps):
                ...
                # apply grad modifier
                grad_mod_loss = self.inner_loss(x=self.update_rule(state, self.grad_mod.M_0*grad, 0), y=y, AE=self.reg_net)
                grad = self.grad_mod(state, grad, grad_mod_loss)
