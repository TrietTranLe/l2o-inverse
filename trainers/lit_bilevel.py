...
class LitBiLevel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y, x = batch
        try:
            if self.l2o.update_rule.mode == "proj_Dl1ball":
                if x is None:
                    raise ValueError("[LitBiLevel] Ground truth x is required for validation when using 'proj_Dl1ball' update rule.")
...
class LitBiLevel_multiA(pl.LightningModule):
    """
    PyTorch Lightning module for bi-level meta-learning.
    Supports multiple L2O submodules with independent learning rates.
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        x, _ = batch
        y = self.l2o.fwd(x, seed=self.global_step)  # to have different fwd for each batch
