...
# def show_decoder_solver(tensor_list):
#     titles = ["State", "P", "Grad", "P * Grad"]
#     num_row = len(titles)
#     num_col = len(tensor_list)

#     fig, axes = plt.subplots(num_row, num_col, figsize=(3 * num_col, 3 * num_row))

#     for i, row in enumerate(tensor_list):
#         for j, tensor in enumerate(row):
#             # Remove the channel dimension if needed: (H, W, 1) -> (H, W)
#             if isinstance(tensor, torch.Tensor):
#                 tensor = tensor.detach().cpu().numpy()
#             image = np.squeeze(tensor)

#             # Handle axes being 1D if there's only one row
#             ax = axes[j][i] if num_col > 1 else axes[j]
#             im = ax.imshow(image)
#             ax.axis('off')

#             # Optional: titles for each subplot
            
#             ax.set_title(titles[j] + "_" + str(i), fontsize=10)
#             plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
#         if len(row) < num_row:
#             for j in range(j+1, num_row):
#                 ax = axes[j][i] if num_col > 1 else axes[j]
#                 ax.axis('off')

#     #plt.tight_layout()
#     filename = f"solver_result.png"
#     plt.savefig(Path(figs_path, filename))
#     plt.close(fig)  # Close to free up memory

# show_decoder_solver(litmodel.solver.gradients)
...
with torch.no_grad():
    # print(f"LSTM: {sum(p.numel() for p in baseline_nets['lstm'].parameters())}")
    # print(f"CNN: {sum(p.numel() for p in baseline_nets['1dcnn'].parameters())}")
...
# data = np.vstack([litmodel.solver.varc, litmodel.solver.obsc, litmodel.solver.pc]).T
# np.savetxt(f"Ours.txt", data, header="varc obsc pc")
